...
def substrings(string)
    ...
    (0..string.length - 1).each do |i|
      (i..string.length - 1).each { |j| subs << string[i..j] }
    end
    subs.uniq
end
...
def subwords(word, dictionary)
    ...
    subs.select { |w| dictionary.include?(w) }
end
...
class Array
  ...
  def my_inject(&blk)
      ...
      res = self[0]
      (1..self.length - 1).each { |i| res = blk.call(res, self[i])}
      res
  end
end
...
def concatenate(strings)
  strings.inject { |sum, ele| sum + ele }
end